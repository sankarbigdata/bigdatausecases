...
def main():
   ...
   print(curts)
   gcs_df.repartition(2).write.json("gs://we41_usecase_bucket/usecase4/cust_output_json_"+curts) #change to your cloud bucket location
   ...
   curts = spark.createDataFrame([1], IntegerType()).withColumn("curts", current_timestamp()).select(date_format(col("curts"), "yyyyMMddHHmmSS")).first()[0]
   print(curts)
   gcs_df.repartition(2).write.mode("overwrite").csv("gs://we41_usecase_bucket/usecase4/cust_csv")  #change to your cloud bucket location
   print("gcs Write Completed Successfully")
